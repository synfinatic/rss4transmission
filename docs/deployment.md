...
services:
  rss4transmission:
    ...
    # (/cancel, /start, /healthz). Set PUBLIC_LISTEN=0.0.0.0:8080 and add:
    # ports:
    #   - "8080:8080"
...
services:
  rss4transmission:
    ...
    environment:
      ...
      - HISTORY_FILE=
      - PRIVATE_LISTEN=
      - PUBLIC_LISTEN=      # /cancel, /start, /healthz (e.g. 0.0.0.0:8080); port-forward from your firewall to this port
      - TORRENT_CACHE_DIR=
...
| `PUBLIC_LISTEN` | `host:port` — public-facing listener for `/cancel`, `/start`, and `/healthz` |
