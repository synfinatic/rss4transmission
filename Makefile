...
DESCRIPTION               := A RSS Feed to Transmission Service
...
run: ./cmd/$(PROJECT_NAME)/*.go  ## build and run cria using $PROGRAM_ARGS
	go run ./cmd/$(PROJECT_NAME)/... $(PROGRAM_ARGS)
...
build-race: .prepare ## Build race detection binary
	go build -race -ldflags='$(LDFLAGS)' -o $(OUTPUT_NAME) ./cmd/$(PROJECT_NAME)/...

debug: .prepare ## Run debug in dlv
	dlv debug ./cmd/$(PROJECT_NAME)/...
